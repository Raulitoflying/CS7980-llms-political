...
class ContextEnhancedClassifier:
    def __init__(self, model_name, profiles_file, output_dir):
        ...
        # Load user profiles
        self.user_profiles = self.load_profiles(profiles_file)
        print(f"Loaded {len(self.user_profiles)} user profiles")
        
        # Track metrics for posts with and without profiles
        self.with_profile_count = 0
        self.without_profile_count = 0
        
        self.output_dir = output_dir

    def load_profiles(self, profiles_file):
        """Load previously generated user profiles"""
        try:
    ...
    def classify_post(self, post, max_retries=3):
        """Classify a post with user profile context if available"""
        username = post['name']
        user_profile = self.user_profiles.get(username, None)
        has_profile = user_profile is not None
        
        # Track metrics
        if has_profile:
            self.with_profile_count += 1
        else:
            self.without_profile_count += 1
        ...
        for attempt in range(max_retries):
            try:
                ...
                if has_profile:
                    # Format the profile into a readable string
                    profile_summary = f"""USER PROFILE SUMMARY:
Username: {user_profile.get('username', username)}
Political Leaning: {user_profile.get('political_leaning', 'unknown')} (Confidence: {user_profile.get('confidence', 'unknown')})
Key Indicators: {', '.join(user_profile.get('key_indicators', []))}
Recurring Topics: {', '.join(user_profile.get('recurring_topics', []))}
Language Style: {user_profile.get('language_style', '')}
Sentiment Patterns: {user_profile.get('sentiment_patterns', '')}
Context Notes: {user_profile.get('context_notes', '')}"""

                    # Create system prompt with user profile context
                    system_prompt = f"""You are a person with Left political stance. Analyze the following discussion group post and classify the author's political orientation.
...
{{
    ...
    "explanation": "A detailed explanation of why you chose this classification based on the content"
                    ...
                    # Standard prompt without user profile
...
Provide your response in this exact JSON format:
{
    ...
    def process_posts(self, posts, subset_size=None):
        ...
        # Reset counters for accurate reporting
        self.with_profile_count = sum(1 for c in classifications if c.get('has_profile', False))
        self.without_profile_count = sum(1 for c in classifications if not c.get('has_profile', False))
        ...
        output = {
            ...
            "total_posts_with_profile": self.with_profile_count,
            "total_posts_without_profile": self.without_profile_count,
            "metrics": metrics,
            "classifications": classifications
        }

        # Save results
        filename = f"context_enhanced_{self.model_name.replace('/', '_')}.json"
        ...
        print(f"With Profile Accuracy: {metrics['with_profile_accuracy']:.2f}%")
        print(f"Without Profile Accuracy: {metrics['without_profile_accuracy']:.2f}%")
        
        if metrics['with_profile_accuracy'] > 0 and metrics['without_profile_accuracy'] > 0:
            improvement = metrics['with_profile_accuracy'] - metrics['without_profile_accuracy']
            print(f"Accuracy Improvement with Profile: {improvement:.2f}%")
    ...
    def calculate_metrics(self, classifications):
        """Calculate comprehensive metrics including with/without profile comparisons"""
        if not classifications:
            print("No data to calculate metrics")
            return {
                'total_predictions': 0,
                'overall_accuracy': 0,
                'with_profile_accuracy': 0,
                'without_profile_accuracy': 0,
                'right': {
                    'precision': 0, 
                    'recall': 0, 
                    'f1_score': 0, 
        ...
        # Counters for with/without profile comparison
        with_profile_correct = 0
        with_profile_total = 0
        without_profile_correct = 0
        without_profile_total = 0

        # Fill the confusion matrix
        for clf in classifications:
            pred_orientation = clf['orientation'].lower()
            true_orientation = clf['true_orientation']
            has_profile = clf.get('has_profile', False)
            ...
            # Track profile usage metrics
            if has_profile:
                with_profile_total += 1
                if pred_orientation == true_orientation:
                    with_profile_correct += 1
            else:
                without_profile_total += 1
                if pred_orientation == true_orientation:
                    without_profile_correct += 1
        ...
        metrics['with_profile_accuracy'] = safe_divide(with_profile_correct, with_profile_total) if with_profile_total > 0 else 0
        metrics['without_profile_accuracy'] = safe_divide(without_profile_correct, without_profile_total) if without_profile_total > 0 else 0
...
# Main execution for comparing our context-enhanced approach with the baseline
def main():
    # File paths
    remaining_data_file = "remaining_0.8_posts.json"
    profiles_file = "political_user_summaries.json"
    ...
    subset_size = 200  # Change this to process more or fewer posts
    
    output_dir = input("Enter the directory name to save the JSON files: ")
    ...
    for model_name in models:
        ...
        classifier = ContextEnhancedClassifier(model_name, profiles_file, output_dir)
        # Process the posts
        results = classifier.process_posts(data['posts'], subset_size)
    
    print("\nContext-enhanced classification complete!")
